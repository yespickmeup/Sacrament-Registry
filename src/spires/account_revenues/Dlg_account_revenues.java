 ...
 * @author Guinness
 */
public class Dlg_account_revenues extends javax.swing.JDialog {
...
//</editor-fold>
    ...
    private Dlg_account_revenues(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        setUndecorated(true);
        initComponents();
        myInit();
    ...
    private Dlg_account_revenues(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        setUndecorated(true);
        initComponents();
        myInit();
    ...
    private Dlg_account_revenues myRef;

    private void setThisRef(Dlg_account_revenues myRef) {
        this.myRef = myRef;
    }
    private static java.util.Map<Object, Dlg_account_revenues> dialogContainer = new java.util.HashMap();
    ...
    public static Dlg_account_revenues create(java.awt.Window parent, boolean modal) {
    ...
    public static Dlg_account_revenues create(java.awt.Window parent, java.awt.Dialog.ModalityType modalType) {

        if (parent instanceof java.awt.Frame) {

            Dlg_account_revenues dialog = dialogContainer.get(parent);

            if (dialog == null) {
                dialog = new Dlg_account_revenues((java.awt.Frame) parent, false);
        ...
        if (parent instanceof java.awt.Dialog) {
            ...
            if (dialog == null) {
                dialog = new Dlg_account_revenues((java.awt.Dialog) parent, false);
                dialog.setModalityType(modalType);
                dialogContainer.put(parent, dialog);
                java.util.logging.Logger.getAnonymousLogger().log(Level.INFO, "instances: {0}", dialogContainer.size());
                dialog.setThisRef(dialog);
    ...
    public static void main(String args[]) {
        ...
        Dlg_account_revenues dialog = Dlg_account_revenues.create(new javax.swing.JFrame(), true);
...
//                btn_0.doClick();
    ...
    public static void loadData_official_schedule_types(List<Account_revenues.to_account_revenues> acc) {
    ...
    public static class Tblofficial_schedule_typesModel extends AbstractTableAdapter {
        ...
        public Object getValueAt(int row, int col) {
            Account_revenues.to_account_revenues tt = (Account_revenues.to_account_revenues) getRow(row);
            switch (col) {
                case 0:
                    return " " + tt.revenue;
                case 1:
    ...
    private void ret_official_schedule_types() {
        ...
        List<Account_revenues.to_account_revenues> datas = Account_revenues.ret_data(where);
    ...
    private void save_official_schedule_types() {
        ...
        if (row < 0) {
            int id = 0;
            String revenue = jTextField1.getText();
            int status = 1;
            Account_revenues.to_account_revenues account = new Account_revenues.to_account_revenues(id, revenue, status);
            Account_revenues.add_data(account);
            Alert.set(1, "");
            ret_official_schedule_types();

        } else {
            Account_revenues.to_account_revenues to = (Account_revenues.to_account_revenues) tbl_official_schedule_types_ALM.get(row);
            int id = to.id;
            String revenue = jTextField1.getText();
            int status = 1;
            Account_revenues.to_account_revenues account = new Account_revenues.to_account_revenues(id, revenue, status);
            Account_revenues.update_data(account);
            Alert.set(2, "");
            ret_official_schedule_types();
        }
        clear_official_schedule_types();
    ...
    private void select_official_schedule_types() {
        ...
        if (col == 1) {
            Account_revenues.to_account_revenues to = (Account_revenues.to_account_revenues) tbl_official_schedule_types_ALM.get(row);
            jTextField1.setText(to.revenue);
        }
        if (col == 2) {
